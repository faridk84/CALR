...
def convert_coordinates(string):
    ...
    """
    return [(lat, int) for (int, lat) in json.loads(string)]
...
def load_data():
    ...
    competition_test_labels_cache = 'cache/competition-test-labels.npy'
    ...
    if os.path.isfile(train_cache):
        ...
        competition_test_labels = np.load(competition_test_labels_cache)
        ...
        train, validation, train_labels, validation_labels = train_test_split(train, train_labels, test_size=0.1)
        #validation, test, validation_labels, test_labels = train_test_split(validation, validation_labels, test_size=0.5)
        
        competition_test['POLYLINE_FULL'] = competition_test['POLYLINE'].copy()  # First, keep old version handy for future reference.
        competition_test['POLYLINE'] = competition_test['POLYLINE'].apply(random_truncate)  # Then truncate.
        competition_test_labels = np.column_stack([
                competition_test['POLYLINE_FULL'].apply(lambda x: x[-1][0]),
                competition_test['POLYLINE_FULL'].apply(lambda x: x[-1][1])
        ])
        ...
        np.save(competition_test_labels_cache, competition_test_labels)
    ...
    data.__dict__.update({
        ...
        'competition_test_labels': competition_test_labels,
